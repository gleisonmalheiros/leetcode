...
public class CyclicRotation {
    ...
    public int[] solution(int[] A, int K) {
        if (K < 1 || A.length == 0) { // nothing to rotate
            return A;
        }
        if (K > A.length) { // avoiding unnecessary interactions
            K = K % A.length;
        }
        if (K == A.length) { // unnecessary rotation
            return A;
        }
        ...
        // starting with the back of the array
